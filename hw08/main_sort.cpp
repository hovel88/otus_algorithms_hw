...
{
    std::cout << "программа оценки работы алгоритмов сортировки (QuickSort, MergeSort) в базовых версиях" << std::endl;
    ...
    for (const auto sz : sizes) {
        {
            QuickSort sort_algo(QuickSort::V1);
            results.emplace_back(_test_func(arr, sz, &sort_algo));
        }
        {
            MergeSort sort_algo(MergeSort::V1);
            results.emplace_back(_test_func(arr, sz, &sort_algo));
        }
    }
