...
Из основных ограничений - класс должен работать со строками, состоящими только из символов `'a'-'z'`. Поэтому была предложена подобная реализация с расчетом индекса по коду символа и определения его позиции в массиве с прямым доступом к элементам.
