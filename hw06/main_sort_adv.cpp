...
{
    std::cout << "программа оценки работы алгоритмов сортировки (BubbleSort, InsertionSort, ShellSort) в оптимизированных версиях" << std::endl;
    ...
    for (const auto sz : sizes) {
        {
            BubbleSort sort_algo(BubbleSort::V2);
            results.emplace_back(_test_func(arr, sz, &sort_algo));
        }
        {
            InsertionSort sort_algo(InsertionSort::V2);
            results.emplace_back(_test_func(arr, sz, &sort_algo));
        }
        {
            InsertionSort sort_algo(InsertionSort::V3);
            results.emplace_back(_test_func(arr, sz, &sort_algo));
        }
        {
            ShellSort sort_algo(ShellSort::V2);
            results.emplace_back(_test_func(arr, sz, &sort_algo));
        }
        {
            ShellSort sort_algo(ShellSort::V3);
            results.emplace_back(_test_func(arr, sz, &sort_algo));
        }
    }
