...
{
    std::cout << "программа оценки работы алгоритмов сортировки (BubbleSort, InsertionSort, ShellSort) в базовых версиях" << std::endl;
    ...
    for (const auto sz : sizes) {
        {
            BubbleSort sort_algo(BubbleSort::V1);
            results.emplace_back(_test_func(arr, sz, &sort_algo));
        }
        {
            InsertionSort sort_algo(InsertionSort::V1);
            results.emplace_back(_test_func(arr, sz, &sort_algo));
        }
        {
            ShellSort sort_algo(ShellSort::V1);
            results.emplace_back(_test_func(arr, sz, &sort_algo));
        }
    }
