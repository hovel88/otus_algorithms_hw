...
{
    std::cout << "программа расчета ходов шахматной фигуры Конь" << std::endl;
    std::string N_str;
    ...
    try {
        ...
        if (N > 63) {
            ...
            ChessKnight knight;
            uint64_t moves = knight.move(N);
            uint32_t count = knight.popcount(moves);
            ChessBoard::print_result(N, moves, count);
            ChessBoard::draw_result(N, moves, knight.literal());
        }
