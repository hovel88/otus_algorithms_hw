...
test_map_t run_tests(tested_1in_1out_func fun, const test_map_t& prepared)
{
...
test_map_t run_tests(tested_1in_2out_func fun, const test_map_t& prepared)
{
    test_map_t test_map(prepared);
    for (auto& t : test_map) {
        const auto in  = _read_file(t.in_file);
        ...
        if (in.size() > 0 && out.size() > 1) {
            t.result = fun(out.at(0), out.at(1), in.at(0));
        }
    }
    _show_results(test_map);
    return test_map;
...
// test_map_t run_tests(tested_2in_1out_func fun, const test_map_t& prepared)
// {
//     test_map_t test_map(prepared);
//     for (auto& t : test_map) {
//         const auto in  = _read_file(t.in_file);
