...
{
    std::cout << "программа расчета ходов шахматной фигуры Слон" << std::endl;
    std::string N_str;
    ...
    try {
        ...
        if (N > 63) {
            ...
            ChessBishop bishop;
            uint64_t moves = bishop.move(N);
            uint32_t count = bishop.popcount(moves);
            ChessBoard::print_result(N, moves, count);
            ChessBoard::draw_result(N, moves, bishop.literal());
        }
