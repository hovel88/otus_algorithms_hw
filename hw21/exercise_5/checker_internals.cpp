...
static void _show_results(const test_map_t& test_map)
{
    ...
    for (const auto& t : test_map) {
        ...
        std::cout << t.num << " (" << std::setw(12) << t.result.elapsed_time.count() << " нс) : "
...
void elapsed_time_table(const std::map<std::string, test_map_t>& tests)
{
    for (const auto& [_, tm] : tests) {
        std::cout << std::left << std::setw(20) << "Group name";
        for (const auto& t : tm) {
            std::cout << std::right << std::setw(15) << t.num;
        }
        std::cout << std::endl;
        break;
    }
    for (const auto& [name, tm] : tests) {
        ...
        for (const auto& t : tm) {
            std::cout << std::right << std::setw(15) << t.result.elapsed_time.count();
        }
        std::cout << std::endl;
    }
}

test_map_t run_tests(tested_func fun, const test_map_t& prepared)
{
