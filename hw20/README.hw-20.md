...
Реализация в методах **_compress()** и **_decompress()**.

Основная проблема наивного подхода в том как кодируются неповторяющиеся символы. Нам нужно понимать разницу между повторяющейся и неповторяющейся последовательностью. Идея улучшения - использовать концепцию контейнера TLV (используется триада `<type><length><value>`). Где T - это тип поля, L - длина данных, V - сами данные как есть. Только в нашем случае T вырождено в один бит (выставлен (0x80) если повторяющаяся последовательность, и сброшен (0x00) если неповторяющаяся), потому что надо разделять только 2 состояния.
...
Улучшенная версия в некоторых случаях не дает никаких результатов, проценты остаются практически такими же. Но в некоторых случаях небольшое сжатие всё же происходит.

Для данного набора тестовых семплов, мне кажется интересным результат, что MP3 и ZIP (это уже сжатые файлы по факту) показали неплохой коэффициент сжатия. В то время как BMP и WAV (пусть это данные и не в сыром виде, но без агрессивной компрессии) практически не дали хорошего коэффициента сжатия. А ещё интересно, что бинарный файл хорошо сжался, видимо так получилось, что он обладает внутри повторяющимися структурами.

## Выводы

* RLE эффективен для данных с длинными повторяющимися последовательностями
* Лучше всего сжимает: текстовые файлы с пробелами, простые графики (где много повторяющихся пикселей)
* Худшие результаты: уже сжатые данные (7Zip, JPG), особенно плохо со случайными данными
